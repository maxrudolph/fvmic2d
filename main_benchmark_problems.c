...
/*     if(!rank) */ ierr = saveRunInfo( &options, &materials, iMonte);
	...
	if( markers[m].cellX != -1){
	  PetscInt mymat = (PetscInt) markers[m].Mat;
	  if( materials.hasDamage[mymat] && materials.hasDilation[mymat] ){
	    if( options.dilationModel == 1 ){
	      ...
	      PetscScalar rho1 = materials.materialRho[mymat]*(1-materials.materialAlpha[mymat]*markers[m].T);/* portion of density due to temperature*/
	      markers[m].rhodot = materials.damagem[mymat]*pow(1.0-markers[m].D,materials.damagem[mymat]-1.0)*-markers[m].Ddot * rho1;	      
	    } else if(options.dilationModel == 2){
	      /* second formulation */	      
	      if( markers[m].D < options.Dcrit){
		/* no density changes until D gets very close to 1 */
		markers[m].rhodot = -materials.materialRho[mymat]*materials.materialAlpha[mymat]*markers[m].Tdot;
	      } else {
		/* drho/dt = rho0*V0/V */
		PetscScalar V = 1+(markers[m].D + options.Dcrit - 1.0)*(Vmax-1.0)/(options.Dcrit);
		/* dV/dt = dV/dD * dD/dt */
		PetscScalar dVdD = (Vmax-1.0)/options.Dcrit;
		/* modify materialRho to account for thermal expansion */
		PetscScalar rho1 = materials.materialRho[mymat]*(1-materials.materialAlpha[mymat]*markers[m].T);/* portion of density due to temperature*/
		//markers.rhodot[m] = -materials.materialRho[mymat]*dVdD*markers.Ddot[m]/(V*V);
		markers[m].rhodot = -rho1*dVdD*markers[m].Ddot/(V*V) -V*(materials.materialRho[mymat]*materials.materialAlpha[mymat]*markers[m].Tdot );
	      }
	    }/* end if then on dilation Model */
	  }
	}/* end if in bounds */
      ...
      /* go through nodal rhodot and make sure that  density is not changing too fast. if it is, reduce compuational timestep*/
      {
	PetscScalar dtlimit = 1e99;
	PetscInt i,j;
	/* get local part of rhodot*/
	PetscScalar **rhodot;
	PetscScalar **rho;
	DMDAVecGetArray(grid.da, nodalFields.rhodot, &rhodot);
	DMDAVecGetArray(grid.da, nodalFields.rho, &rho);
	PetscInt x,y,m,n;
	ierr=DMDAGetCorners(grid.da,&x,&y,PETSC_NULL,&m,&n,PETSC_NULL);CHKERRQ(ierr);
	for(i=x;i<x+m;i++){
	  for(j=y;j<y+n;j++){
	    if(rhodot[j][i] !=0.0){
	      PetscScalar dt1 = fabs(0.01*rho[j][i]/rhodot[j][i]);
	      if( dt1 < dtlimit) dtlimit=dt1;
	    }
	  }
	}
	DMDAVecRestoreArray(grid.da,nodalFields.rhodot,&rhodot);
	DMDAVecRestoreArray(grid.da,nodalFields.rho,&rho);
	PetscScalar dtmin;
	MPI_Allreduce(&dtlimit,&dtmin,1,MPI_DOUBLE,MPI_MIN,PETSC_COMM_WORLD);/* find global minimum timestep*/
	dtlimit=dtmin;
	if( dtlimit < options.dtMax){
	  if(!rank) printf("maximum dt allowed for 0.01 density change is %e years\n",dtlimit/(365.25*24*3600));
	  if(dtlimit < 1.0) {	
