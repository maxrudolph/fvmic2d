...
int main(int argc, char **args){
  ...
  /* begin bisection loop for determining critical thickness */
  PetscInt nTime = problem.options.nTime; /* number of timesteps to run for each test case */
  PetscInt maxIter = 10;
  PetscScalar Dmin = 0.0;
  PetscScalar Dmax = problem.options.LY;
  PetscScalar delta = 500.0; /* tolerance for bisection */ 
  PetscInt done = 0;
  PetscInt iter =0;


  while( iter<maxIter && ((Dmax-Dmin)>delta)){
    PetscScalar D = (Dmin+Dmax)/2.0;
    problem.options.LX = 2.0*D;
    problem.options.nTime = nTime;
    problem.options.LY = D;
    problem.options.dtMax = problem.options.LY*problem.options.LY/1e-6/100.0; /* set maximum timestep based on thermal diffusion */
    problem.options.totalTime = problem.options.LY*problem.options.LY/1e-6/16.0;
    if(!rank) printf("Bisection: Dmin = %e, Dmax = %e, LY = %e, dtMax = %e\n",Dmin,Dmax,problem.options.LY,problem.options.dtMax);

    PetscScalar vrms[10000];
    /* end setup options */
    ...
    ierr=DMCreateMatrix(problem.grid.vda,MATAIJ,&problem.mech_system.lhs);CHKERRQ(ierr);
    ...
    ierr=DMCreateMatrix(problem.grid.da,MATAIJ,&problem.z_system.lhs);CHKERRQ(ierr);
    ...
    ierr=DMCreateMatrix(problem.grid.da,MATAIJ,&problem.thermal_system.lhs);CHKERRQ(ierr);
	...
	ierr = DMCreateMatrix(problem.grid.vda,MATAIJ, &problem.mech_system.pc); CHKERRQ(ierr);
    ...
    if(!rank) ierr = saveRunInfo( &problem.options, &problem.materials, iter);
	...
	ierr = rmsVelocity( &problem.grid, &problem.nodal_fields, &vrms[iTime] );CHKERRQ(ierr);
  ...
  nextMonte:;
	...
	ierr = MatDestroy(&problem.mech_system.pc);CHKERRQ(ierr);
      ...
      /* check to see whether vrms[itime] > vrms[0] */
      if( vrms[iTime] > 5.0*vrms[0] ){
      /* Dcrit is smaller than this value */
	if(!rank) printf("Bisection: Convection detected\n");
	Dmax = D;
    }else{/* Dcrit is larger than this value */
	if(!rank) printf("Bisection: No convection detected\n");
	Dmin = D;
    }
      if(!rank) printf("Bisection: D=%e, vrms[0] = %e, vrms[iTime]= %e, elapsedTime=%e\n, iTime = %d\n",D,vrms[0],vrms[iTime],elapsedTime,iTime);
      if(!rank) printf("Bisection: now looking for Dcrit between [%e,%e]\n",Dmin,Dmax);
      iter++;
