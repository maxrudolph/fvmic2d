...
npy = 4;
% left side
xt = linspace(0.25,0.77,npx);
%xt1 = xt(1:end-1) + diff(xt)/2;
xt1=xt
yt = linspace(0.3,1,npy+1);
...
for jy=1:npy
    for ix=1:npx
        if( mask(jy,ix) )
            ...
            tex1 = texture(midx(jy,ix,1));
            for isum=2:nsum
               tex1.ctheta=[tex1.ctheta; texture(midx(jy,ix,isum)).ctheta];
               tex1.cphi=[tex1.cphi; texture(midx(jy,ix,isum)).cphi];
            end
            
            plotTexturePoleFigureMTEX(tex1,1,800); set(gca,'Visible','off','DataAspectRatio',[1 1 1]);            
            p=get(gca,'pos')
        end
    end
end
