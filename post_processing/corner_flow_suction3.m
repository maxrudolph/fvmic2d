...
% loadgrid
output_dir = '~/subduction_test53/output';
...
%     xlabel('Distance from wedge corner (km)');
    ...
    %% Make figure showing velocity magnitude
NYY = length(newvx(:,1));
NXX = length(newvx(1,:));
for i=1:NYY
    for j=1:NXX
        vm(i,j) = sqrt(newvx(i,j)^2 + newvy(i,j)^2);
    end
end
% 
% figure;
% imagesc(vm)
    ...
    % cut off melting in regions with vm < tol.
    tol = .1e-9;
    for i=1:NYY
        for j=1:NXX
            if vm(i,j) < tol
                melt_production(i,j) = 0;
            end
        end
    end
