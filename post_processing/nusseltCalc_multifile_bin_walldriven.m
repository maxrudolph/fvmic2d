...
PETSC_DIR=pd(1:end-1);
setenv('PETSC_DIR',PETSC_DIR);
addpath([PETSC_DIR '/bin/matlab']);
...
for iFile=1:length(files)
   ...
   elapsedTime = nf.elapsedTime;
    ...
    for i=1:nf.NY-1
        ...
        dtdx(i) = 1/2*(T(i,2)-T(i,1) + T(i+1,2)-T(i+1,1));
        dtdx(i) = dtdx(i)/(grid.x(2)-grid.x(1));
        %weight by cell width
        %     dtdz(i) = dtdz(i)*
        dy(i)=(grid.y(i+1)-grid.y(i));
    end
    %Deffinition from eq 10 of Turan et al. 2010
    Nu(iFile) = abs((sum(dy.*dtdx))/LY*LY/(nf.T(1,end)-nf.T(1,1)))
