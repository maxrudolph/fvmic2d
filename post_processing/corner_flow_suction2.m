...
%     set(gca,'YLim',[0 160]);    
    set(gca,'XLim',[0 440]);   % set(gca,'XLim',[0 560]);
    set(gca,'YLim',[0 160]);
%     xlabel('Distance from wedge corner (km)');
    ...
    interpmethod='linear';
    newT = interp2(nf.gridx,nf.gridy,nf.T,X,Y,interpmethod)-273;
    
%     newvx = scatteredInterpolant(nf.gridx(:),nf.gridyc(:),nf.vx(:));
    newvx = interp2(nf.gridx,nf.gridyc,nf.vx,X,Y,interpmethod);
    newvy = interp2(nf.gridxc,nf.gridy,nf.vy,X,Y,interpmethod);
    newp = interp2(nf.gridxc,nf.gridyc,nf.p,X,Y,interpmethod);
    ...
    for i = 1:dd;
        for j = 1:ww;
            meltf(i,j) = KatzMelt(newtotp(i,j)/1e9, newT(i,j), 0.1);
        end
    end
    
    meltff = meltf;    % repeat last column and row to calculate centers
    newtotpp = newtotp;
    newTT = newT;
    meltff(end+1,end+1) = meltff(end,end); 
    newtotpp(end+1,end+1) = newtotpp(end,end);
    newTT(end+1,end+1) = newTT(end,end);
    
    for i = 1:dd;
        for j = 1:ww;
        dfdp(i,j) = (meltff(i+1,j+1) - meltff(i,j))/(newtotpp(i+1,j+1) - newtotpp(i,j));
        dfdT(i,j) = (meltff(i+1,j+1) - meltff(i,j))/(newTT(i+1,j+1) - newTT(i,j));
        end
    end
%     meltf = interp2( melt_table.T, melt_table.P*1e9, melt_table.F, newT, newtotp ,interpmethod,0.0);
%     dfdp = interp2( melt_table.T, melt_table.P*1e9, melt_table.dFdP, newT, newtotp,interpmethod,0.0);
%     dfdT = interp2( melt_table.T, melt_table.P*1e9, melt_table.dFdT, newT, newtotp,interpmethod,0.0);
    ...
    for i = 1:dd;
        for j = 1:ww;
            dpdx(i,j) = (newtotpp(i+1,j) - newtotpp(i,j))/dx;
            dpdy(i,j) = (newtotpp(i,j+1) - newtotpp(i,j))/dy;
            dTdx(i,j) = (newTT(i+1,j) - newTT(i,j))/dx;
            dTdy(i,j) = (newTT(i,j+1) - newTT(i,j))/dy;
        end
    end
