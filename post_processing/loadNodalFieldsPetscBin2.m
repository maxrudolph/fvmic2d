...
% #endif
%   ierr=  VecView( nodalFields->soxx,viewer);CHKERRQ(ierr);        18
%   ierr=  VecView( nodalFields->soyy,viewer);CHKERRQ(ierr);        19
%   ierr=  VecView( nodalFields->sozz,viewer);CHKERRQ(ierr);        20
%   ierr=  VecView( nodalFields->soxy,viewer);CHKERRQ(ierr);        21
%   ierr=  VecView( nodalFields->soxz,viewer);CHKERRQ(ierr);        22
%   ierr=  VecView( nodalFields->soyz,viewer);CHKERRQ(ierr);        23
%   /* strain rate components */
%   ierr=  VecView( nodalFields->edotxx,viewer);CHKERRQ(ierr);      24
%   ierr=  VecView( nodalFields->edotyy,viewer);CHKERRQ(ierr);      25
%   ierr=  VecView( nodalFields->edotxy,viewer);CHKERRQ(ierr);      26
%   ierr=  VecView( nodalFields->edotxz,viewer);CHKERRQ(ierr);      27
%   ierr=  VecView( nodalFields->edotyz,viewer);CHKERRQ(ierr);      28
% 
%   wxy 29
%   wxz 30
%   wyz 31
...
%   ierr=  VecView( nodalFields->ha,viewer);CHKERRQ(ierr);          32
% #ifdef TEXTURE
...
nf.ha = reshape(nf1{30+oft},[NX NY])';
nf.exx = reshape(nf1{24+oft},[NX NY])';
nf.eyy = reshape(nf1{25+oft},[NX NY])';
nf.exy = reshape(nf1{26+oft},[NX NY])';
nf.exz = reshape(nf1{27+oft},[NX NY])';
nf.eyz = reshape(nf1{28+oft},[NX NY])';

nf.sxx = reshape(nf1{18+oft},[NX NY])';
nf.syy = reshape(nf1{19+oft},[NX NY])';
nf.szz = reshape(nf1{20+oft},[NX NY])';
nf.sxy = reshape(nf1{21+oft},[NX NY])';
nf.sxz = reshape(nf1{22+oft},[NX NY])';
nf.syz = reshape(nf1{23+oft},[NX NY])';

nf.wxy = reshape(nf1{29+oft},[NX NY])';
...
if( length(nf1) >= 34+oft )
    if(length(nf1{33+oft}) == NX*NY*2)
        tmp = reshape(nf1{33+oft},[2 NX NY]);
