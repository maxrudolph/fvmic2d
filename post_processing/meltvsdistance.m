...
melt_production = melt_production*dA;
melt_production(melt_production<0) = 0;
melt_production(isnan(melt_production)) = 0;
...
plot(newx/1e3,melt_vol,'LineWidth',2,'Color',[ 0.7500         0    0.7500])
hold on;
...
hleg = legend('Decoupling no root','Thickened root w/ decoupling','Thinned lithosphere','Thickened root (50 K hotter)','Thickened root (deep decouple)','Location','NW');
% hleg = legend('Reference case','Thickened root','Decoupling no root','Thickened root w/ decoupling','Thickened root deep decoupling','Thinned lithosphere', 'Location','NW');
% hleg = legend('Decoupling no root','5 km','10 km','15 km','20 km','Location','NW');
% hleg = legend('290 km','240 km','190 km','Location','NE');
set(hleg,'FontSize',12)
set(gca,'FontSize',12)
set(gcf, 'Color', 'w');

melt_total = sum(meltsum(1:97))
% melt_total = sum(meltsum(1:121))
