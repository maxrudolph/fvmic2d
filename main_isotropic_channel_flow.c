...
int main(int argc, char **args){
  ...
  ierr = initializeIrregularGridConstantInnerOuter( &grid, options.LX, options.LY, options.NX, options.NY, options.gridRatio);CHKERRQ(ierr);
  ierr = saveGrid( &grid );
  

  /* initialize scaling parameters*/
  PetscInt ndof = grid.NX*grid.NY*3;  
  PetscScalar Kbond = 4*options.etamin/((grid.dx+grid.dy)*(grid.dx+grid.dy));
  PetscScalar Kcont = 2*options.etamin/(grid.dx+grid.dy);
  ...
  ierr = initializeNodalFields( &nodalFields, &grid, &options);CHKERRQ(ierr);
  {
    ...
    int x,y,z,m,n,p;
    ierr=DAGetCorners(grid.da,&x,&y,&z,&m,&n,&p); CHKERRQ(ierr);
  ...
  ierr=DAGetISLocalToGlobalMapping(grid.da, &ltogm);CHKERRQ(ierr);
  

  /* initialize LHS,LHSz matrix and RHS,RHSz matrix for mechanical problem */
  ierr=DAGetMatrix(grid.vda,MATAIJ,&LHS);CHKERRQ(ierr);
  ierr = PetscObjectSetName( (PetscObject) LHS,"CmechLHS");
  ierr=DAGetMatrix(grid.da,MATAIJ,&LHSz);CHKERRQ(ierr);
  ...
  for(iMonte=0;iMonte<options.nMonte;iMonte++){
	...
	ywt=(1-ymid)-(1-ymid)*pow(fabs(ywt-ymid)/(ymid),10);
      ...
      markers[m].T = (options.TTop+(options.TBtm-options.TTop)*pow(ywt,2+xwt) + options.TBtm+(options.TTop-options.TBtm)*pow(1-ywt,3-xwt))/2;
      markers[m].Mat = 0;
    ...
    for(iTime=iTime0;iTime<options.nTime;iTime++){
	...
	if(!rank) PetscPrintf(PETSC_COMM_SELF,"Global memory use %e\n",memuset); 
      ...
      PetscScalar Vmax = 1.0/options.maxPorosity;
      for(m=0;m<markerset.nMark;m++){
	...
	if(fn) {
	  ...
	  PetscLogStagePush(stages[11]);
	  /* 	  saveGriddedMarkersBinary( &markers, &grid, 5*grid.NX, 5*grid.NY,iMonte,iTime); */
	  /* 	    ierr=saveMarkersBinary( &markers, iMonte,iTime); */
	...
	ierr=nodalStressStrain(&grid, &nodalFields, displacementdt,nodalHeating, options.gy);CHKERRQ(ierr);
	...
	PetscLogStagePush(stages[12]); ierr=updateMarkerStrainPressure( &grid,&nodalFields, &markerset, &materials, displacementdt); PetscLogStagePop();
	...
	/* end repeat temperature solution */
      ...
      ierr=subgridTemperatureChanges(thermalS, &grid, &nodalFields, &markerset, &materials,dt, &options);
      PetscLogStagePop();
	...
	//saveGriddedMarkersBinary( &markers, &grid, 5*grid.NX, 5*grid.NY,iMonte,iTime,elapsedTime);
      ...
      /* advect markers */
      ierr=advectMarkersRK(&markerset,&nodalFields, &grid,dt);
      elapsedTime+=dt;
 ...
 abort:;
  ...
  ierr=DARestoreGlobalVector(grid.vda,&RHS);CHKERRQ(ierr);
  ierr=DARestoreGlobalVector(grid.da,&RHSz);CHKERRQ(ierr);
  ierr=DARestoreGlobalVector(grid.da,&thermalRHS);CHKERRQ(ierr);
