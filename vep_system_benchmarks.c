/* this is a special version of the vep_system routine that can enforce internal velocity boundary conditions for the sandbox benchmark */
...
PetscErrorCode formVEPSystem(NodalFields *nodalFields, GridData *grid, Mat LHS, Vec RHS, Mat LHSz, Vec RHSz, PetscScalar *Kbond, PetscScalar *Kcont, PetscScalar gy, PetscScalar dt, Options *options, BoundaryValues *bv){
...
#ifdef SANDBOX
  /* Special internal boundary conditions for sandbox problem */
  //% Mobile Wall
  /*     if (markers[m].X>grid->LX - 0.01 && markers[m].X<grid->LX-0.005 && markers[m].Y>0.005 && markers[m].Y<grid->LY-0.002){ */
  /*       markers[m].Mat = 4; */
  /*     } */
  PetscScalar wallxmin = grid->LX - 0.01;
  PetscScalar wallxmax = grid->LX - 0.005;
  PetscScalar wallymin = 0.005;
  PetscScalar wallymax = grid->LY-0.002;
  PetscScalar wallvx = -2.5/100.0/3600.0;
  PetscScalar wallvy = 0.0;
#endif
  ...
  /* xz - soxz is currently still located at basic nodes */
  ...
  for(i=y1;i<y+n;i++){
    for(j=x1;j<x+m;j++){
      PetscScalar dx1=grid->x[j]-grid->x[j-1];
      PetscScalar dy1=grid->y[i]-grid->y[i-1];
      PetscScalar rhobar = (rho[i-1][j-1]+rho[i-1][j]+rho[i][j-1]+rho[i][j])*0.25;
      PetscScalar rhodotbar =  (rhodot[i-1][j-1] + rhodot[i-1][j]+rhodot[i][j-1]+rhodot[i][j])*0.25;
      
      //      printf("rhobar = %e, rhodotbar = %e\n",rhobar,rhodotbar);
      drhodtl += 1.0/rhobar*rhodotbar*dx1*dy1;
    }
  }
  /* */
  PetscScalar drhodtglobal;
  /* This allreduce sums the local density changes to get a global flux of material required to */
  /* globally satisfy continuity */
  ...
  PetscScalar inflow = drhodtglobal/LX + outflow/LX; 
 
  if(!rank) printf("global drhodt = %e, inflow across one element = %e\n",drhodtglobal,inflow);
  ...
  for(jy=-1;jy<ng;jy++){
	...
	pdof[jy][ix] = 3*globalIdx[ix+mg*jy]+0;
	vxdof[jy][ix] = 3*globalIdx[ix+mg*jy]+1;
	vydof[jy][ix] = 3*globalIdx[ix+mg*jy]+2;
  ...
  for(jy=y;jy<y+n;jy++){
    ...
    for(ix=x;ix<x+m;ix++){
      ...
      /**              %continuity*/		       
      if( (grid->xperiodic && jy == 0) || (!grid->xperiodic && (jy == 0 || ix == 0)) ){/* no ghost nodes on left boundary if the domain is periodic */
	...
	ierr = VecSetValue(RHS,pdof[jyl][ixl],0.0,INSERT_VALUES); CHKERRQ(ierr);
      ...
      } else if( !grid->xperiodic && 0 && ((jy==1 && ix == 1) || (jy==NY-1 && ix==1))){
	//upper and lower left corner - dp/dx = 0;
	//LHS(p(i,j),p(i,j)) = 1.0*Kbond;
	ierr = MatSetValue(LHS,pdof[jyl][ixl],pdof[jyl][ixl], 1.0*Kbond[0],INSERT_VALUES); CHKERRQ(ierr);
	//      LHS(p(i,j),p(i,j+1)) = -1.0*Kbond;
	/* 	ierr = MatSetValue(LHS,pdof,pdof + 3*NY,-1.0*Kbond[0],INSERT_VALUES);CHKERRQ(ierr); */
	ierr = MatSetValue(LHS,pdof[jyl][ixl],pdof[jyl][ixl+1],-1.0*Kbond[0],INSERT_VALUES);CHKERRQ(ierr);
	//      RHS(p(i,j)) = 0.0;
	ierr = VecSetValue(RHS,pdof[jyl][ixl],0.0,INSERT_VALUES); CHKERRQ(ierr);
	
      } else if( !grid->xperiodic && 0 && ((jy==1 && ix==NX-1) || (jy==NY-1 && ix== NX-1))) {
	...
	ierr = VecSetValue(RHS,pdof[jyl][ixl],0.0,INSERT_VALUES); CHKERRQ(ierr);
      ...
      } else if( jy==1 && ix==2 ){/* pressure specified in one cell */
...
#ifdef SANDBOX
	//}else if( ix > 1 && ix < NX-1 && grid->x[ix-1] <= wallxmin && grid->x[ix] >= wallxmin && grid->yc[jy+1] >= wallymin && grid->yc[jy+1] <= wallymax){
	// }else if( ix == NX-1-7 && jy >= 6 && jy <= NY-1-3){
      }else if( ix > 1 && jy < NY-1 &&  grid->x[ix] >= wallxmin && grid->x[ix-1] < wallxmin && grid->yc[jy+1] >= wallymin && grid->yc[jy+1] <= wallymax){
	//printf("setting vx node %d,%d as internal boundary\n",ix,jy);fflush(stdout);
	/* internal boundary for moving wall */
	ierr = MatSetValue(LHS, vxdof[jyl][ixl], vxdof[jyl][ixl],1.0*Kbond[0],INSERT_VALUES);CHKERRQ(ierr);
	ierr = VecSetValue(RHS, vxdof[jyl][ixl], Kbond[0]*wallvx,INSERT_VALUES);CHKERRQ(ierr);      
#endif
      }else{
...
#ifdef SANDBOX
	//      }else if( ix>1 && ix < NX-2 && grid->xc[ix-1] <= wallxmin && grid->xc[ix+1] >= wallxmin && grid->y[jy] >= wallymin && grid->y[jy] <= wallymax){
	/* internal boundary for moving wall */
	//}else if( ix == NX-1-7 && jy == NY-1-3 ){
      }else if( ix > 1 && jy<NY-2&& grid->xc[ix+1] >= wallxmin && grid->xc[ix] < wallxmin && grid->y[jy] <= wallymax && grid->y[jy+1] > wallymax){
	
	//printf("setting vy node %d,%d as internal boundary\n",ix,jy);fflush(stdout);
	ierr = MatSetValue(LHS, vydof[jyl][ixl], vydof[jyl][ixl],1.0*Kbond[0],INSERT_VALUES);CHKERRQ(ierr);
	ierr = VecSetValue(RHS, vydof[jyl][ixl], Kbond[0]*wallvy,INSERT_VALUES);CHKERRQ(ierr);      
#endif
      }else {
	...
	/*   etaA=(etaS(i-1,j) + etaS(i,j))/2; */
	/* 	PetscScalar etaA = (etaSZ[jy-1][ix-1] + etaSZ[jy-1][ix])/2.0; */
	/*             etaB=(etaS(i-1,j-1)+etaS(i,j-1))/2; */
	/* 	PetscScalar etaB = (etaSZ[jy-1][ix] + etaSZ[jy][ix])/2.0; */
	/*             etaC=(etaS(i,j-1)+etaS(i,j))/2; */
	/* 	PetscScalar etaC = (etaSZ[jy][ix] + etaSZ[jy][ix-1])/2.0; */
	/*             etaD=(etaS(i-1,j-1)+etaS(i-1,j))/2; */
	/* 	PetscScalar etaD = (etaSZ[jy][ix-1] + etaSZ[jy-1][ix-1])/2.0; */
	...
	PetscScalar Rval = -gz*(rho[jy][ix]+rho[jy-1][ix-1]+rho[jy][ix-1]+rho[jy-1][ix])/4.0 - \
	  /* 	  (soxzZ[jy-1][ix]-soxzZ[jy-1][ix-1])/dx - (soyzZ[jy][ix-1]-soyzZ[jy-1][ix-1])/dy; *//* this is the version for when soxz is evaluated at vx nodes */
  ...
  for(jy=y;jy<y+n;jy++){
	...
	if( bv->mechBCTop.type[0] == 0){/* kinematic boundary condition */
	...
	if( bv->mechBCLeft.type[1] == 0 ){/* kinematic */
	...
	}else if( bv->mechBCLeft.type[1] ==1 ){/* no slip */
	...
	if( bv->mechBCRight.type[2] == 0 ){/* prescribed velocity */
	  //	  PetscScalar etaB = (etaSZ[jy][ix] + etaSZ[jy-1][ix])/2.0;
	  PetscScalar etaB = etavxZ[jy-1][ix];
