...
PetscErrorCode projectMarkersNodesAll(MarkerSet *markerset, GridData *grid, NodalFields *nodalFields, Materials *materials, Options *options){
  ...
  Vec rhol, rhodotl, etaSl, etaNl, kThermall,Tl,Cpl,muNl,muSl,soxxl,soyyl,soxyl,hal;
  ...
  //  PetscScalar **wtvy;
  /* field variables: rho,etaS,etaN,kThermal,T,Cp,muN,muS,soxx,soxy,soyy,sozz,soxy,soxz,soyz*/
  PetscScalar **rho,**rhodot, **etaS, **etaN, **kThermal,**T,**Cp,**muN,**muS,**soxx,**soyy,**soxy,**ha;
#ifdef TEXTURE
...
/*   ierr=DMDAVecGetArray( grid->da,wyzl,&wyz); CHKERRQ(ierr); */
  ...
  for( m=0;m<markerset->nMark;m++){
    if(markers[m].cellX != -1){/* check marker in domain*/
      ...
      PetscInt irk;
      PetscScalar mvx[4], mvy[4], mwxy[4];//, mwxz[4], mwyz[4];
	...
	if(grid->xperiodic){
      ...
      markers[m].s.T11 = msxx0*pow( cos(thetaz), 2.0 ) + msyy0*pow( sin(thetaz), 2.0 ) - msxy0 *sin(2.0*thetaz);
      markers[m].s.T22 = msxx0*pow( sin(thetaz), 2.0 ) + msyy0*pow( cos(thetaz), 2.0 ) + msxy0 *sin(2.0*thetaz);
      markers[m].s.T12 = 0.5*(msxx0 - msyy0)* sin(2.0*thetaz) + msxy0 * cos( 2.0*thetaz );
...
/* 				   sin(thetay)*(-2*msxy0*cos(2*thetaz) + (-msxx0 + msyy0)*sin(2*thetaz))))/2.; */
	...
	for(c2=0;c2<NT;c2++){
      ...
      markers[m].Eii += dt*sqrt( 0.5*(markers[m].e.T11*markers[m].e.T11 + markers[m].e.T22*markers[m].e.T22 ) + markers[m].e.T12*markers[m].e.T12 );
